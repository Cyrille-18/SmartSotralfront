...
export class LignesComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.lignes.set([
      { id: 1, trackingId: 'l1', numero: '1', depart: 'Agoè', arrive: 'Assigamé' },
      { id: 2, trackingId: 'l2', numero: '3', depart: 'Port', arrive: 'Adidogomé' },
    ]);
  }
  ...
  save(): void {
    ...
    if (this.editingId()) {
      this.lignes.update(list => list.map(l => l.trackingId === this.editingId() ? { ...l, ...payload } : l));
    } else {
      const nextId = Math.max(...this.lignes().map(l => l.id ?? 0), 0) + 1;
      this.lignes.update(list => [...list, { id: nextId, trackingId: `l${nextId}`, ...payload }]);
    }
  ...
  remove(ligne: Ligne): void {
    this.lignes.update(list => list.filter(l => l.trackingId !== ligne.trackingId));
  }
}
