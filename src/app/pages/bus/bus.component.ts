...
export class BusComponent implements OnInit {
  ...
  loadBuses(): void {
    this.busService.getAll().subscribe({
      next: buses => this.busList.set(buses),
      error: () => this.busList.set([]),
    });
  }
  ...
  save(): void {
    ...
    const req$ = this.editingTrackingId()
      ? this.busService.update(this.editingTrackingId()!, payload as Bus)
      : this.busService.create(payload as Bus);

    req$.subscribe({
      next: () => { this.loading.set(false); this.closeModal(); this.loadBuses(); },
      error: () => { this.loading.set(false); },
    });
  }

  delete(bus: Bus): void {
    if (!bus.trackingId) return;
    this.loading.set(true);
    this.busService.delete(bus.trackingId).subscribe({
      next: () => { this.loading.set(false); this.loadBuses(); },
      error: () => { this.loading.set(false); },
    });
  }
}
